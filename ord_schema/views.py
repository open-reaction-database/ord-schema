...
* ``reaction_smiles`` is the stored REACTION_SMILES identifier when present, and
  otherwise generated by ``message_helpers.get_reaction_smiles``.
...
def _component_smiles(reaction: reaction_pb2.Reaction) -> tuple[list[str], list[str]]:
    """Returns ``(input_smiles, output_smiles)``, skipping components without SMILES.
    ...
    for key in sorted(reaction.inputs):
        for compound in reaction.inputs[key].components:
            smiles = message_helpers.get_compound_smiles(compound)
            if smiles:
                inputs.append(smiles)
    outputs = []
    for outcome in reaction.outcomes:
        for product in outcome.products:
            smiles = message_helpers.get_compound_smiles(product)
            if smiles:
...
def reaction_smiles(reaction: reaction_pb2.Reaction) -> str | None:
    """Returns the reaction's SMILES, generating one if it is not stored.

    Generation is best-effort: a reaction whose components cannot produce a SMILES reads
    as null rather than failing the whole view.
    """
    try:
