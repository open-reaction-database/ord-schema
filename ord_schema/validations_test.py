...
class ValidationsTest(parameterized.TestCase, absltest.TestCase):
    ...
    def test_units_should_fail(self, message, expected_error):
        with self.assertRaisesRegex(validations.ValidationError, expected_error):
    ...
    def test_synthesized_compound(self):
        ...
        with self.assertRaisesRegex(validations.ValidationError, "only .* when SYNTHESIZED"):
            self._run_validation(message)
    ...
    def test_reaction(self):
        ...
        with self.assertRaisesRegex(validations.ValidationError, "reaction input"):
            self._run_validation(message)
    ...
    def test_bad_reaction_smiles(self):
        ...
        with self.assertRaisesRegex(validations.ValidationError, "requires at least two > characters"):
            self._run_validation(message)
    ...
    def test_reaction_recursive(self):
        ...
        with self.assertRaisesRegex(validations.ValidationError, "reaction outcome"):
            self._run_validation(message, recurse=False)
        ...
        with self.assertRaisesRegex(validations.ValidationError, "require an amount"):
            self._run_validation(message)
        ...
        with self.assertRaisesRegex(validations.ValidationError, "INTERNAL_STANDARD"):
            self._run_validation(message)
    ...
    def test_missing_provenance(self):
        ...
        with self.assertRaisesRegex(validations.ValidationError, "requires provenance"):
            self._run_validation(message, recurse=False, options=options)

    def test_data(self):
        message = reaction_pb2.Data()
        with self.assertRaisesRegex(validations.ValidationError, "requires one of"):
            self._run_validation(message)
        message.bytes_value = b"test data"
        with self.assertRaisesRegex(validations.ValidationError, "format is required"):
            self._run_validation(message)
    ...
    def test_dataset_example(self):
        ...
        with self.assertRaisesRegex(validations.ValidationError, "description is required"):
            self._run_validation(message)
        message.description = "test example"
        with self.assertRaisesRegex(validations.ValidationError, "url is required"):
            self._run_validation(message)
        message.url = "example.com"
        with self.assertRaisesRegex(validations.ValidationError, "created is required"):
            self._run_validation(message)
    ...
    def test_dataset_crossreferences(self):
        ...
        with self.assertRaisesRegex(validations.ValidationError, "undefined reaction_ids"):
            self._run_validation(message)
