...
Options:
    ...
    --no-validate           If set, do not run validations on reactions
"""
# import glob
...
def main(kwargs):
    ...
    logger.info(
        "*** Important message - The Open Reaction Database repository, ord-data, uses the CC-BY-SA license"
        " for all data. "
    ...
    if not os.path.isfile(inputfile):
        logger.info(
            "Error - Conversion failed. Please check that the --input file exists at the specified location."
        )
        exit(1)
    ...
    if "UDM" not in udm_reactions:
        logger.info(
            "Error - Input file is not UDM format - please ensure the <UDM> tag is used as the root of your "
            ".xml file."
        )
        exit(1)
    ...
    for reaction in udm_reactions_list:
        ...
        if "RXNSTRUCTURE" in reaction:
            ...
            for structure in rxnstructures:
                ...
                pb2_reaction.identifiers.add(
                    type=ordtype, details=orddetails, value=udmvalue
                )
        ...
        for variation in variations:
            ...
            for reactant in reactants:
                ...
                if "MOLECULE" in reactant:
                    ...
                    if "molblock" in molval:
                        molcomponent.identifiers.add(
                            type="MOLBLOCK", details="MOLECULE -> MOLSTRUCTURE from UDM"
                        )
            ...
            if (
                ...
                and "CONDITION_GROUP" in variation["CONDITIONS"]
            ):
                pb2_conditions = reaction_pb2.ReactionConditions()
                if "TEMPERATURE" in variation["CONDITIONS"]["CONDITION_GROUP"]:
                    if (
                        "exact"
                        in variation["CONDITIONS"]["CONDITION_GROUP"]["TEMPERATURE"]
                    ):
                        pb2_reaction.conditions.temperature.setpoint.value = float(
                            variation["CONDITIONS"]["CONDITION_GROUP"]["TEMPERATURE"][
                                "exact"
                            ]
                        )
                if "PRESSURE" in variation["CONDITIONS"]["CONDITION_GROUP"]:
                    if (
                        "exact"
                        in variation["CONDITIONS"]["CONDITION_GROUP"]["PRESSURE"]
                    ):
                        pb2_reaction.conditions.pressure.setpoint.value = float(
                            variation["CONDITIONS"]["CONDITION_GROUP"]["PRESSURE"][
                                "exact"
                            ]
                        )
                if "STIRRING" in variation["CONDITIONS"]["CONDITION_GROUP"]:
                    pb2_reaction.conditions.stirring.details = variation["CONDITIONS"][
                        "CONDITION_GROUP"
                    ]["STIRRING"]
                if "REFLUX" in variation["CONDITIONS"]["CONDITION_GROUP"]:
                    pb2_reaction.conditions.reflux = variation["CONDITIONS"][
                        "CONDITION_GROUP"
                    ]["REFLUX"]
                if "PH" in variation["CONDITIONS"]["CONDITION_GROUP"]:
                    if "exact" in variation["CONDITIONS"]["CONDITION_GROUP"]["PH"]:
                        pb2_reaction.conditions.ph = float(
                            variation["CONDITIONS"]["CONDITION_GROUP"]["PH"]["exact"]
                        )

                pb2_setup = reaction_pb2.ReactionSetup()
                if "PREPARATION" in variation["CONDITIONS"]["CONDITION_GROUP"]:
                    pb2_reaction.setup.environment = variation["CONDITIONS"][
                        "CONDITION_GROUP"
                    ]["PREPARATION"]
        ...
        if "VARIATION" in reaction and "PRODUCT" in reaction["VARIATION"]:
            ...
            for udm_product in products:
                ...
                if "YIELD" in udm_product:
                    ...
                    product_measurement.type = (
                        reaction_pb2.ProductMeasurement.ProductMeasurementType.YIELD
                    )
                    product_measurement.float_value.value = float(
                        udm_product["YIELD"]["exact"]
                    )
        ...
        if "LEGAL" in udm_reactions:
            pb2_reaction.provenance.experimenter.organization = udm_reactions["LEGAL"][
                "PRODUCER"
            ]
        if "VARIATION" in reaction and "SCIENTIST" in reaction["VARIATION"]:
            pb2_reaction.provenance.experimenter.name = reaction["VARIATION"][
                "SCIENTIST"
            ]
        if "LEGAL" in udm_reactions:
            pb2_reaction.provenance.record_created.person.organization = udm_reactions[
                "LEGAL"
            ]["PRODUCER"]
        if "VARIATION" in reaction and "SCIENTIST" in reaction["VARIATION"]:
            pb2_reaction.provenance.record_created.person.name = reaction["VARIATION"][
                "SCIENTIST"
            ]
        ...
        if (
            "LEGAL" in udm_reactions
            and "DOI" in udm_reactions["LEGAL"]
            and "CITATIONS" not in reaction
        ):
            pb2_reaction.provenance.doi = udm_reactions["LEGAL"]["DOI"]
        elif (
            "CITATIONS" in udm_reactions
            and "VARIATION" in reaction
            and "CITATION" in reaction["VARIATION"]
        ):
            ...
            for citation in udm_reactions["CITATIONS"]["CITATION"]:
                ...
                if (
                    citation["@ID"] == variation_citation["@CIT_ID"]
                    and "DOI" in citation
                ):
        ...
        if "CITATIONS" in reaction:
            ...
            pb2_reaction.provenance.patent = reaction["CITATIONS"][0]["CITATION"][
                "PATENT_NUMBER"
            ]
        ...
        if "VARIATION" in reaction and "CREATION_DATE" in reaction["VARIATION"]:
            pb2_reaction.provenance.record_created.time = reaction["VARIATION"][
                "CREATION_DATE"
            ]

        if "VARIATION" in reaction and "MODIFICATION_DATE" in reaction["VARIATION"]:
            pb2_reaction.provenance.record_modified.time = reaction["VARIATION"][
                "MODIFICATION_DATE"
            ]
    ...
    # Assign canonical dataset_id/reaction_ids and provenance updates.
    ...
    logger.info(
        "Conversion completed successfully! Check ORD_UDM_CONVERSION_"
        + inputfile
        + ".debug.json file for errors."
    )
