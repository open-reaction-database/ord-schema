...
def migrate_stirring_conditions(old: reaction_old_pb2.StirringConditions, new: reaction_pb2.StirringConditions):
...
def migrate_electrochemistry_conditions(
    ...
    migrate_messages(old, new, ("current", "voltage", "electrode_separation", "cell"))
    migrate_repeated_messages(old, new, ("measurements",))


def migrate_flow_conditions(old: reaction_old_pb2.FlowConditions, new: reaction_pb2.FlowConditions):
...
def migrate_setup(old: reaction_old_pb2.ReactionSetup, new: reaction_pb2.ReactionSetup):
...
def migrate_conditions(old: reaction_old_pb2.ReactionConditions, new: reaction_pb2.ReactionConditions):
    ...
    if old.HasField("electrochemistry"):
        migrate_electrochemistry_conditions(old.electrochemistry, new.electrochemistry)
...
def migrate_workup(old: reaction_old_pb2.ReactionWorkup, new: reaction_pb2.ReactionWorkup):
...
def migrate_dataset(old_dataset: dataset_old_pb2.Dataset) -> dataset_pb2.Dataset:
    """Migrates a single Dataset."""
    new_dataset = dataset_pb2.Dataset()

    # Copy unchanged dataset-level information.
    migrate_scalars(old_dataset, new_dataset, ("name", "description", "dataset_id"))
    ...
    for old in old_dataset.reactions:
        ...
        migrate_repeated_messages(old, new, ("identifiers", "observations", "outcomes"))
        ...
        if new.SerializeToString(deterministic=True) != old.SerializeToString(deterministic=True):
            num_changed += 1
...
def main(argv):
    ...
    for filename in filenames:
        ...
        old_dataset = message_helpers.load_message(filename, dataset_old_pb2.Dataset)
        ...
        if num_changed:
            ...
            new_filename = os.path.join(FLAGS.output_dir, os.path.basename(filename))
