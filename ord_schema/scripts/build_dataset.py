...
def main(argv):
    ...
    if not FLAGS.description:
        logging.warning("Consider setting the dataset description with --description")
    dataset = dataset_pb2.Dataset(name=FLAGS.name, description=FLAGS.description, reactions=reactions)
    if FLAGS.validate:
