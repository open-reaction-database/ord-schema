...
FLAGS = flags.FLAGS
flags.DEFINE_string("template", None, "Path to a Reaction pbtxt file defining a template.")
flags.DEFINE_string(
    "spreadsheet",
    None,
    "Path to a spreadsheet file (with a header row) defining " "values to replace placeholders in the template.",
)
...
def main(argv):
    ...
    logging.info("generating new Dataset from %s and %s", FLAGS.template, FLAGS.spreadsheet)
    dataset = templating.generate_dataset(template_string, df, validate=FLAGS.validate)
    if FLAGS.output:
